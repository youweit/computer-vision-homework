...
function main () {
  cv.readImage(process.argv[2], function (err, inputMat) {
    ...
    let globalLabel = 1
    let connectedPointValue = 255
    let label = new Array(height)
    ...
    for (let k = 0; k < boundingBoxes.length; k++) {
      ...
      if (boundingBoxes[k].area > 500) {
        ...
        if (boundingBoxes[k].label > 0) {
          for (let j = boundingBoxes[k].left; j <= boundingBoxes[k].right; j++) {
            binary.pixel(j, boundingBoxes[k].top, boxColor)
            binary.pixel(j, boundingBoxes[k].down, boxColor)
          }
          for (let j = boundingBoxes[k].top + 1; j <= boundingBoxes[k].down - 1; j++) {
            binary.pixel(boundingBoxes[k].left, j, boxColor)
            binary.pixel(boundingBoxes[k].right, j, boxColor)
          }
          // cendroid
          let x = boundingBoxes[k].totalX / boundingBoxes[k].area
          let y = boundingBoxes[k].totalY / boundingBoxes[k].area
